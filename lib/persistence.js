...
const stmts = {
  ...
  renameTag: db.prepare('UPDATE tags SET name = ? WHERE name = ?'),
  deleteTag: db.prepare('DELETE FROM tags WHERE name = ?'),
  allTagsCount: db.prepare(`
    SELECT t.name, COUNT(lt.link_id) as usageCount
    FROM tags t
...
function renameTag(oldName, newName) {
  const info = stmts.renameTag.run(newName, oldName);
  return info.changes > 0;
}

function deleteTag(name) {
  const info = stmts.deleteTag.run(name);
  return info.changes > 0;
}

function getAllTags() {
  return stmts.allTagsCount.all();
}
