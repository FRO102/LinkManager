...
function createBackup() {
  try {
    // Checks both tables — a collection with only notes (no links at all)
    // still has data worth preserving, and VACUUM INTO backs up the whole
    // database file either way, so skipping the backup just because
    // `links` is empty would silently leave notes-only installs with no
    // automatic backups at all.
    ...
    if (linkCount === 0 && noteCount === 0) return; // nothing to preserve
