...
db.exec(`
  ...
  -- Triggers to keep links_fts in sync
  CREATE TRIGGER IF NOT EXISTS links_ai AFTER INSERT ON links BEGIN
    INSERT INTO links_fts(rowid, title, url, description) VALUES (new.rowid, new.title, new.url, new.description);
  END;
  CREATE TRIGGER IF NOT EXISTS links_ad AFTER DELETE ON links BEGIN
    INSERT INTO links_fts(links_fts, rowid, title, url, description) VALUES('delete', old.rowid, old.title, old.url, old.description);
  END;
  ...
  SELECT rowid, title, url, description FROM links
  WHERE rowid NOT IN (SELECT rowid FROM links_fts);
...
function migrateFromLegacyJson() {
  ...
  try {
    ...
    logger.error('[migration] Could not parse legacy links.json, skipping migration:', err.message);
    return;
  }
  ...
  logger.info(`[migration] Importing ${legacyLinks.length} link(s) from legacy links.json into SQLite...`);
  ...
  fs.renameSync(LEGACY_JSON_FILE, LEGACY_JSON_FILE + '.bak');
  logger.info(`[migration] Done. Old file kept as ${path.basename(LEGACY_JSON_FILE)}.bak for reference.`);
}
