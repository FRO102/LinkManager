...
const { IMPORT_MAX_ITEMS } = require('../../../core/lib/config');
const db = require('../../../core/lib/db');
const {
  getLinkById, insertLink, deleteLinkById, nextOrderValue,
  setTagsForLink, searchLinks,
} = require('../persistence');
const { isValidUrl } = require('../../../core/lib/ssrf-guard');
const { normalizeUrlForCompare } = require('../../../core/lib/url-utils');
const { checkLinkStatus, runCheckAll, isCheckInProgress, cancelCheckAll } = require('../link-check');
const { simpleRateLimit } = require('../../../core/lib/rate-limit');
...
router.get('/', (req, res) => {
  ...
  if (q) {
    ...
    // Fallback to reading all links if no search query
