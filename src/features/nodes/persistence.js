...
function getAllUrls() {
  return db.prepare('SELECT url FROM links').all().map(r => r.url);
}
...
function searchLinks(query) {
  const ftsStmt = db.prepare(`
    ...
    ORDER BY rank
  `);
  const rowids = ftsStmt.all(query).map(r => r.rowid);

  if (rowids.length === 0) return [];

  // Fetch full link data for the matched rowids
  const linksStmt = db.prepare(`
    SELECT * FROM links
    WHERE rowid IN (${rowids.map(() => '?').join(',')})
    ORDER BY "order" ASC
  `);
  const rows = linksStmt.all(...rowids);
