...
const { OG_FETCH_TIMEOUT_MS, OG_CACHE_TTL_MS, OG_CACHE_FAILURE_TTL_MS, CHECK_USER_AGENT, OG_CACHE_MAX_SIZE } = require('../../core/lib/config');
...
async function getOgDataCached(url) {
  ...
  // Prevent memory leak: implement a simple FIFO eviction policy if cache grows too large
  if (ogCache.size >= OG_CACHE_MAX_SIZE) {
